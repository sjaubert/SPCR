...
Or nous savons qu'un estimateur sans biais de $\sigma²$ est :
\[S_{n-1}^{2}=\frac{n}{n-1}S_{n}^{2}\] 

où \[S_{n}^{2}=\frac{1}{n}\sum_{i=1}^{n}(X_{1}-\bar{X})^{2}\] 
et donc \[U=\frac{nS_{n}^{2}}{\sigma²}=\frac{(n-1)S_{n-1}^{2}}{\sigma²}\] suit une loi du $\chi^{2}$ à $n-1$ ddl

De plus on peut écrire : $\frac{U}{n-1}=\frac{S_{n-1}^{2}}{\sigma²}$

Par conséquent nous pouvons écrire :

\[\frac{\bar{X}-\mu}{\frac{\sigma}{\sqrt{n}}} =\frac{\bar{X}-\mu}{\frac{S_{n-1}}{\sqrt{n}}}\]
\[=\frac{\bar{X}-\mu}{\frac{S_{n-1}}{\sqrt{n}}}\]
\[  =\frac{(\bar{X}-\mu) \;\sigma}{\frac{\sigma}{\sqrt{n}} S_{n-1}} =\frac{Z\sigma}{S_{n-1}}\]
\[=\frac{Z}{\frac{S_{n-1}}{\sigma}}=\frac{Z}{\sqrt{\frac{U}{n-1}}}\]

Par conséquent $\frac{\bar{X}-\mu}{\frac{\sigma}{\sqrt{n}}}$ suit bien la loi de Student à $(n-1)$ à $n-1$ ddl
